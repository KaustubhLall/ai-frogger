...
static void update_logs(FroggerEnv* env) {
    ...
    for (int lane = 0; lane < env->config.river_lanes; lane++) {
        for (int i = 0; i < s->log_count[lane]; i++) {
            ...
            if (log->speed > 0 && log->x > s->width + 2) {
                log->x = -log->length - 2.0f;
            } else if (log->speed < 0 && log->x + log->length < -2) {
                log->x = (float)(s->width + 2);
            }
        }
    }
}
...
static void update_cars(FroggerEnv* env) {
    ...
    for (int lane = 0; lane < env->config.road_lanes; lane++) {
        for (int i = 0; i < s->car_count[lane]; i++) {
            ...
            if (car->speed > 0 && car->x > s->width + 2) {
                car->x = -car->length - 2.0f;
            } else if (car->speed < 0 && car->x + car->length < -2) {
                car->x = (float)(s->width + 2);
            }
        }
    }
}
    ...
    if (tile == TILE_RIVER) {
        for (int lane = 0; lane < env->config.river_lanes; lane++) {
            ...
            for (int i = 0; i < s->log_count[lane]; i++) {
                LogState* log = &s->logs[lane][i];
                if (!log->active) continue;
                if ((float)s->frog.x + 0.5f >= log->x &&
                    (float)s->frog.x + 0.5f <= log->x + log->length) {
                    s->frog.on_log = 1;
                    s->frog.x = (int)(log->x + s->frog.log_offset);
                    if (s->frog.x < 0 || s->frog.x >= s->width) {
                        /* Carried off screen */
                        s->frog.alive = 0;
                    }
                    return;
    ...
    if (reason != TERMINAL_NONE) {
        ...
        switch (reason) {
            case TERMINAL_GOAL:
                s->goals_reached++;
                break;
            case TERMINAL_DEATH_CAR:
                env->last_reward.death_car = env->config.death_penalty;
                s->frog.alive = 0;
                s->total_deaths++;
                break;
            case TERMINAL_DEATH_WATER:
                env->last_reward.death_water = env->config.drowning_penalty;
                s->frog.alive = 0;
                s->total_deaths++;
                break;
            case TERMINAL_TIMEOUT:
                env->last_reward.timeout = env->config.timeout_penalty;
                break;
            default: break;
        }
        env->last_reward.total += env->last_reward.death_car + env->last_reward.death_water +
                                    env->last_reward.timeout;
...
void env_observe(const FroggerEnv* env, Observation* obs) {
    obs_compute(&env->state, obs, &env->config);
}
...
void env_get_debug_snapshot(const FroggerEnv* env, DebugSnapshot* out) {
    ...
    out->last_action = ACTION_WAIT;
    out->decision_text[0] = '\0';
    out->episode = 0;
    out->total_episodes = 0;
    out->cumulative_reward = 0.0f;
}
